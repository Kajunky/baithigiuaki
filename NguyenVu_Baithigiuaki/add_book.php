...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($title) && !empty($author_id) && !empty($category_id) && !empty($publisher) && !empty($publish_year) && !empty($quantity)) {
        $stmt = $mysqli->prepare("INSERT INTO books (title, author_id, category_id, publisher, publish_year, quantity) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("siissi", $title, $author_id, $category_id, $publisher, $publish_year, $quantity);
        $stmt->execute();
        $success_message = "You've successfully added a book";
    } else {
...
<div class="container mt-5">
    <div class="add-book-full">
    <h1 class="text-center text-warning">Add Book</h1>
    ...
    <form method="POST" action="add_book.php">
        ...
        <button class="custom-btn btn-10">Add Book</button>
    </form>
    </div>
</div>
<script>
